    ...
    <pull-down>
      ...
      <div class="read-content" v-show="!loading" :bg="bg_color" :night='bg_night' ref="reader">
        ...
        <div class="chapterContent">
          <p v-for="(item,index) in book.content.split('-')" :key="index">{{item}}</p>
        </div>
        <ul class="tab">
...
<script>
  ...
  import {fetch} from '../store/local.js'
  ...
  export default {
    ...
    methods: {
      getBook(id, chapter) {
        ...
        this.$http.get(`/api/book?book=${id}&id=${chapter}`)
          .then((res) => {
            this.$store.dispatch('toggleLoading')
            this.book = res.data
            console.log("res :"+res)
            console.log("this.book :"+this.book)
            this.book.content = res.data.content
            console.log('this.book.content:~~~'+this.book.content)
          })
      },
      // 上一章
      prevChapter() {
